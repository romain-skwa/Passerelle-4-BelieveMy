...
export const newCreatorData = async (username, email, password, passwordconfirm) => {
        // If a field is empty
        ...
        try { // Add the new data about the users
            // 1 -- Verify if this email is already used
            // Select the "créateurs" collection
            let user = await db.collection("créateurs").find({email}).limit(1).toArray();
            ...
            let pseudo = await db.collection("créateurs").find({username}).limit(1).toArray();
