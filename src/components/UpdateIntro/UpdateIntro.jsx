...
const UpdateIntro = ({ game, fetchgameData, setLoading }) => {
  ...
  useEffect(() => {
    ...
    if (game && game.nameofgame) { const decodedNameOfGame = decodeURIComponent(game.nameofgame); setNameOfGameUpdate(decodedNameOfGame);}
    if (game && game.shortIntroduction) {const encodedShortIntroduction = he.decode(game.shortIntroduction); setShortIntroductionUpdate(encodedShortIntroduction);}
    if (game && game.content) {const encodedIntroductionOfTheGameUpdate = he.decode(game.content); setIntroductionOfTheGameUpdate(encodedIntroductionOfTheGameUpdate);}
    ...
    if (game && game.releaseDate) {
        ...
        if (!isNaN(date.getTime())) { // Vérifiez si la date est valide
    ...
    const onPrepare = async (e) => {
      ...
      try {
        ...
        if (releaseDateUpdate) {
          ...
          if (!datePattern.test(releaseDateUpdateString)) {return toast.error("La date de sortie doit être au format jj/mm/aaaa (ex: 17/05/2025)");}
  ...
  return (
    ...
    >
        ...
        {/* Nom */}
        <div className="text-3xl">Il vous est possible de modifier la description de votre jeu</div>
        <input
            type="text"
        ...
        <div className="flex justify-center">
            <button
              ...
              disabled={
                !nameOfGameUpdate || nameOfGameUpdate.length < 1 ||
                !introductionOfTheGameUpdate || introductionOfTheGameUpdate.length < 1 ||
                platformUpdate < 1 ||
                !shortIntroductionUpdate || shortIntroductionUpdate.length < 1 ||
