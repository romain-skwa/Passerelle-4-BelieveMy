...
export default function WeAreUpdatingData({ comparaison, session }) {
    const { language } = useLanguage();

    return (
        <section className="flex flex-col w-full h-screen justify-center items-center">
            <section className="w-[95vw] tablet:w-[50vw] bg-black">
                {session?.user?.username && (
                    <div className={`text-white text-center uppercase ${vt323.className} text-2xl mb-6`} style={{ letterSpacing: '0.1em' }}>
                        {session?.user.username}
                    </div>
                )}
                ...
                {comparaison.isNameOfGameChanged && (
                    <div className={`text-white text-center uppercase ${vt323.className} text-xl mt-4`} style={{ letterSpacing: '0.1em' }}>
                        {language === "fr"
                            ? "Le nom du jeu"
