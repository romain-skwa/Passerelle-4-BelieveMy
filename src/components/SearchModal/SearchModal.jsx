...
export default function SearchModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 flex justify-center bg-black bg-opacity-50 z-50 text-black">
      ...
      <div className={`mt-4 cursor-not-allowed ${!(selectedGenres.length > 0 || selectedPlatforms.length > 0 || searchTerm.length > 0) 
        ? 'opacity-50 ' 
        : 'opacity-100 '}`}>
        <button
          className={`bg-green-500 px-4 py-2 rounded text-white ${!(selectedGenres.length > 0 || selectedPlatforms.length > 0 || searchTerm.length > 0) 
            ? 'cursor-not-allowed' 
            : 'cursor-pointer'}`}
          onClick={handleSearchClick}
          disabled={!(selectedGenres.length > 0 || selectedPlatforms.length > 0 || searchTerm.length > 0)} // Désactive le bouton si la condition n'est pas remplie
        >
          Rechercher
        </button>
      </div>

        <button onClick={onClose} className="mt-2 bg-red-500 px-4 py-2 rounded text-black">
          Fermer
        </button>
      </div>
    </div>
