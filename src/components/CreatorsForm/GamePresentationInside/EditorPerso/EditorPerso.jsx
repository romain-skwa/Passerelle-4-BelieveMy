...
function MyTextArea({
  ...
  const refUseClickOutside = useClickOutside(() =>
    setIsColorPickerVisible(false)
  ); // Fermer palette de couleurs des lettres
  const refUseClickOutside2 = useClickOutside(() =>
    setIsBackgroundColorPicker(false)
  );
  ...
  const toggleBackgroundColorPicker = () => {
    setIsBackgroundColorPicker((prev) => !prev);
  }; // Inverser la visibilité
  ...
  const handleH2Click = () => {
    ...
    const startIndex = textarea.value.indexOf(
      selectedText,
      textarea.selectionStart
    );
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      ...
      if (offset === startIndex) {
        ...
        if (isH3) {
          ...
          // Si c'est déjà un <h2 , remplacer h2 par span
          newStartIndex = offset;
  ...
  const handleFontSizeChange = (e) => {
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      // Vérifier si c'est l'occurrence que nous voulons formater
      if (offset === startIndex) {
        // font-style: italic; text-decoration: underline; text-decoration: line-through;
        const isStyled =
          p1.includes("font-weight: bold;") ||
          p1.includes("font-style: italic;") ||
          p1.includes("text-decoration: underline;") ||
          p1.includes("text-decoration: line-through;") ||
          p1.includes("color: #");
        const isFontSizeSpan =
          p1.includes("font-size:") &&
          p1.includes("</span>") &&
          (!p1.includes("</h2>") ||
            !p1.includes("</h3>") ||
            !p1.includes("</p>")) &&
          !isStyled;
        const isH2H3PnoStyle =
          (p1.startsWith("<h2>") ||
            p1.startsWith("<h3>") ||
            p1.startsWith("<p>")) &&
          !p1.includes("span") &&
        ...
        if (newSize === "1rem") {
          if (isFontSizeSpan) {
            console.log(
              "(isFontSizeSpan) : Font-size dans un span. Pas dans un h2, h3 ou P. Aucun autre style On supprime le style font-size et les balises span"
            );
            ...
            console.log(
              "(isH2H3PfontSize) : Supprimer les balises h2, h3 ou p ainsi que 'font-size' et sa valeur"
            );
            // Supprimer les balises <h2>, <h3>, <p> tout en conservant le texte à l'intérieur
            newEndIndex = offset + p1.length;
            return p1
              .replace(/<(h2|h3|p)[^>]*>(.*?)<\/(h2|h3|p)>/g, "$2")
              .replace(/font-size:\s*[^;]*;?\s*/g, ""); // Supprime les balises et font-size
          } else {
            // Supprimer font-size: et sa valeur tout en conservant le reste de la sélection
            console.log(
              "Supprimer uniquement 'font-size' et sa valeur. On conserve le reste"
            );
            newEndIndex = offset + p1.length;
        ...
        if (newSize !== "1rem") {
          if (isH2H3PnoStyle) {
            console.log(
              "isH2H3PnoStyle, dans un h2, h3 ou P sans aucun style, Ajouter font-size"
            );
            // Dans un H2, un H3 ou un P sans aucun style, Ajouter style="font-size: ${newSize};" à la balise
            const tagName = p1.includes("<h2>")
              ? "h2"
              : p1.includes("<h3>")
              ? "h3"
              : "p";
            newEndIndex =
              offset +
              p1.length +
              (`<${tagName} style="font-size: ${newSize};">`.length +
                `</${tagName}>`.length);
            return p1.replace(
              /<(h2|h3|p)(.*?)>(.*?)<\/\1>/g,
              `<$1 style="font-size: ${newSize};"$2>$3</$1>`
            );
          } else if (isH2H3PwithStyle) {
            console.log(
              `isH2H3PwithStyle, Dans un H2, un H3 ou un P AVEC style, Ajouter style="font-size: ${newSize};" à la balise`
            );
            // Dans un H2, un H3 ou un P AVEC style, Ajouter style="font-size: ${newSize};" à la balise
            newEndIndex =
              offset + p1.length + `style="font-size: ${newSize};`.length;
            return p1.replace(/style="/, `style="font-size: ${newSize}; `);
          } else if (isfont) {
            console.log(
              "isfont, s'il font-size existe déjà, Changer sa valeur"
            );
            // Trouver la valeur actuelle de font-size
            const currentFontSizeMatch = p1.match(/font-size:\s*([^;]*);?/);
            const currentFontSize = currentFontSizeMatch
              ? currentFontSizeMatch[0]
              : ""; // 'font-size: X;' ou une chaîne vide
            ...
            newEndIndex =
              offset + p1.length - currentFontSizeLength + newFontSizeLength;
            return p1.replace(/font-size:\s*[^;]*;?/g, newFontSize);
          } else if (spanStyled) {
            console.log(
              "spanStyled, Dans un span avec au moins un style, Ajouter font-size"
            );
            ...
            newEndIndex =
              offset +
              `<span style="font-size: ${newSize};">${p1}</span>`.length;
  ...
  const handleBoldClick = () => {
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      ...
      if (offset === startIndex) {
        // font-style: italic; text-decoration: underline; text-decoration: line-through;
        const notIUS =
          !p1.includes("<i>") && !p1.includes("<u>") && !p1.includes("<s>");
        ...
        if (isBold) {
          ...
          console.log(
            `Le texte contient déjà une balise de titre ou de paragraphe, ajouter le Gras.`
          );
          const tagName = p1.includes("<h2>")
            ? "h2"
            : p1.includes("<h3>")
          ...
          newEndIndex =
            offset +
            p1.length +
            (`<${tagName} style="font-weight: bold;">`.length +
              `</${tagName}>`.length); // Ajuster la longueur pour la balise
          return p1.replace(
            /(<\/?)(h[2-3]|p)([^>]*>)/,
            `$1$2 style="font-weight: bold;"$3`
          ); // Ajouter le style à la balise existante
        } else if (isH2H3PBold) {
          // Si le texte contient déjà des balises <h2>, <h3> ou <p>, RETIRER le style de Gras
          console.log(
            `isH2H3PBold ; Le texte contient déjà une balise de titre ou de paragraphe, RETIRER le Gras.`
          );
          const tagName = p1.includes("<h2>")
            ? "h2"
            : p1.includes("<h3>")
          ...
          console.log(
            "otherStyleButBold ; Il y a d'autres styles et on y ajoute du gras"
          );
          ...
          console.log(
            "boldAndOtherStyle ; Il y a déjà des styles en plus de Bold et on va retirer font-weight: bold "
          );
          newEndIndex = newStartIndex + p1.length + 19; // 19 = longueur de 'font-weight: bold;'
          ...
          // Si le texte a déjà d'autres styles en plus de bold, retirer font-weight: bold
          console.log(
            "spanBoldOnly ; On est dans une span contenant Bold et on va retirer la balise <span> et font-weight: bold "
          );
          newEndIndex = newStartIndex + p1.length + 19; // 19 = longueur de 'font-weight: bold;'
          ...
          console.log(
            "spanBoldAndOther ; On est dans une span contenant Bold et d'autres style. Il faut effacer bold"
          );
          newEndIndex = newStartIndex + p1.length + 19; // 19 = longueur de 'font-weight: bold;'
          // Retirer font-weight: bold; aux styles existants
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      ...
      if (offset === startIndex) {
        ...
        if (isItalic) {
          ...
          console.log(
            `Le texte contient déjà une balise de titre ou de paragraphe, ajouter l'italique.`
          );
          const tagName = p1.includes("<h2>")
            ? "h2"
            : p1.includes("<h3>")
          ...
          newEndIndex =
            offset +
            p1.length +
            (`<${tagName} style="font-style: italic ;">`.length +
              `</${tagName}>`.length); // Ajuster la longueur pour la balise
          return p1.replace(
            /(<\/?)(h[2-3]|p)([^>]*>)/,
            `$1$2 style="font-style: italic;"$3`
          ); // Ajouter le style à la balise existante
        } else if (isH2H3PIitalic) {
          // Si le texte contient déjà des balises <h2>, <h3> ou <p>, RETIRER le style d'italique
          console.log(
            `isH2H3PIitalic ; Le texte contient déjà une balise de titre ou de paragraphe, RETIRER l'italique.`
          );
          const tagName = p1.includes("<h2>")
            ? "h2"
            : p1.includes("<h3>")
          ...
          console.log(
            "otherStyleButItalic ; Il y a d'autres styles et on y ajoute de l'italique"
          );
          ...
          console.log(
            "italicAndOtherStyle ; Il y a déjà des styles en plus de l'italique et on va retirer font-style: italic "
          );
          ...
          // Si le texte a déjà d'autres styles en plus d'italique, retirer font-style: italic
          console.log(
            "spanItalicOnly ; On est dans une span contenant Italique et on va retirer la balise <span> et font-style: italic "
          );
          newEndIndex = newStartIndex + p1.length + 19; // 19 = longueur de 'font-style: italic;'
          // Retirer font-style: italic ainsi que les balises <span>
          const content = p1.replace(
            /<span[^>]*style="[^"]*font-style:\s*italic\s*;?[^"]*"[^>]*>(.*?)<\/span>/,
            "$1"
          );
          return content;
          ...
          console.log(
            "spanItalicAndOther ; On est dans une span contenant Italique et d'autres styles. Il faut effacer italique"
          );
          newEndIndex = newStartIndex + p1.length + 19; // 19 = longueur de 'font-style: italic;'
          // Retirer font-style: italic; aux styles existants
  ...
  const handleUnderlineClick = () => {
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      if (offset === startIndex) {
        const notIUS =
          !p1.includes("<u>") && !p1.includes("<i>") && !p1.includes("<s>");
        ...
        if (isUnderlined) {
          ...
          newEndIndex =
            offset +
            p1.length +
            (`<${tagName} style="text-decoration: underline;">`.length +
              `</${tagName}>`.length);
          return p1.replace(
            /(<\/?)(h[2-3]|p)([^>]*>)/,
            `$1$2 style="text-decoration: underline;"$3`
          );
          ...
          newEndIndex = newStartIndex + p1.length + 30;
          const content = p1.replace(
            /<span[^>]*style="[^"]*text-decoration:\s*underline\s*;?[^"]*"[^>]*>(.*?)<\/span>/,
            "$1"
          );
          return content;
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      ...
      if (offset === startIndex) {
        ...
        const isH2H3PStrikeThrough =
          ...
          !p1.includes("font-size:") &&
          !p1.includes("color: #");
        const otherStyleButStrikeThrough =
          !p1.includes("text-decoration: line-through;") &&
          (p1.includes("font-style: italic;") ||
            p1.includes("font-weight: bold;") ||
            p1.includes("font-size:") ||
            p1.includes("color: #"));
        const strikeThroughAndOtherStyle =
          p1.includes("text-decoration: line-through;") &&
          (p1.includes("font-style: italic;") ||
            p1.includes("font-weight: bold;") ||
            p1.includes("font-size:") ||
            p1.includes("color: #"));
        const spanStrikeThroughOnly =
          p1.includes("<span") &&
          p1.includes("</span>") &&
          p1.includes("text-decoration: line-through;") &&
          !p1.includes("<b>") &&
          ...
          !p1.includes("font-size:") &&
          !p1.includes("color: #");
        const spanStrikeThroughAndOther =
          p1.includes("<span") &&
          p1.includes("</span>") &&
          p1.includes("text-decoration: line-through;") &&
          (p1.includes("font-weight: bold;") ||
        ...
        if (isStrikeThrough) {
          ...
          console.log(
            `Le texte contient déjà une balise de titre ou de paragraphe, ajouter le barré.`
          );
          const tagName = p1.includes("<h2>")
            ? "h2"
            : p1.includes("<h3>")
          ...
          newEndIndex =
            offset +
            p1.length +
            (`<${tagName} style="text-decoration: line-through;">`.length +
              `</${tagName}>`.length);
          return p1.replace(
            /(<\/?)(h[2-3]|p)([^>]*>)/,
            `$1$2 style="text-decoration: line-through;"$3`
          );
          ...
          console.log(
            "otherStyleButStrikeThrough ; Il y a d'autres styles, et on y ajoute le barré"
          );
          ...
          console.log(
            "strikeThroughAndOtherStyle ; Il y a déjà d'autres styles en plus du barré, retirer le barré"
          );
          newEndIndex = newStartIndex + p1.length + 30; // 30 = longueur de 'text-decoration: line-through;'
          ...
          newEndIndex = newStartIndex + p1.length + 30;
          const content = p1.replace(
            /<span[^>]*style="[^"]*text-decoration:\s*line-through;\s*;?[^"]*"[^>]*>(.*?)<\/span>/,
            "$1"
          );
          return content;
  ...
  const handleColorChange = (newColorText) => {
    ...
    const otherStyle =
      selectedText.includes("font-weight: bold;") ||
      selectedText.includes("font-style: italic;") ||
      selectedText.includes("text-decoration: underline;") ||
      selectedText.includes("text-decoration: line-through;") ||
      selectedText.includes("font-size:") ||
      selectedText.includes("background-color: #");
    const isColorAndStyle =
      (selectedText.includes('style="color:') ||
        selectedText.includes("; color:")) &&
      otherStyle;
    const isH2H3P =
      selectedText.includes("<h2 ") ||
      selectedText.includes("<h3 ") ||
      (selectedText.includes("<p ") && !otherStyle);
    ...
    if (newColorText === "") {
      ...
      if (isColorAndStyle) {
        ...
        console.log(
          "(isColorAndStyle - couleur de texte) ; Il y a Color + au moins un autre style. Réinitialisation de color : on retire color: #******"
        );
        newEndIndex =
          newStartIndex +
          (selectedText,
          selectedText.replace(/color:\s*#[0-9A-Fa-f]{6};?\s*/, "")).length;
        newText = textarea.value.replace(
          selectedText,
          selectedText.replace(/color:\s*#[0-9A-Fa-f]{6};?\s*/, "")
        );
      } else if (isH2H3P) {
        // Si le texte sélectionné est dans des balises h2, h3 ou p, on le garde tel quel
        console.log(
          "(isH2H3P) Réinitialisation de la couleur de texte. La sélection est dans un h2, h3 ou <p>. Et elle ne contient pas d'autres styles que color."
        );
        newEndIndex =
          newStartIndex +
          (selectedText,
        ...
        console.log(
          "Réinitialisation de color. Suppression des balises <span> </span> et du style color"
        );
        newEndIndex =
          newStartIndex +
          (selectedText,
  ...
  const handleColorClick = (newColorText) => {
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      // Vérifier si c'est l'occurrence que nous voulons formater
      if (offset === startIndex) {
        const isTextColorChanged =
          selectedText.includes("color:") &&
          selectedText.includes("</span>") &&
          !p1.includes("background-color: #");
        const isStyle =
          p1.includes("font-weight: bold;") ||
          p1.includes("font-style: italic;") ||
          p1.includes("text-decoration: underline;") ||
        ...
        const isHeadingOrParagraph =
          p1.includes("<h2>") || p1.includes("<h3>") || p1.includes("<p>"); // Sans couleur
        const isH2H3Pcolored =
          (p1.includes("<h2 ") || p1.includes("<h3 ") || p1.includes("<p ")) &&
          selectedText.includes("color:") &&
          !p1.includes("background-color: #"); // Avec couleur
        newStartIndex = offset;

        if (isHeadingOrParagraph) {
          // Si le texte contient déjà des balises <h2>, <h3> ou <p>, ajouter le style de couleur
          console.log(
            `Le texte contient déjà une balise de titre ou de paragraphe, ajouter le style de couleur.`
          );
          const tagName = p1.includes("<h2>")
            ? "h2"
            : p1.includes("<h3>")
          ...
          newEndIndex =
            offset +
            p1.length +
            (`<${tagName} style="color: ${newColorText};">`.length +
              `</${tagName}>`.length); // Ajuster la longueur pour la balise
          return p1.replace(
            /(<\/?)(h[2-3]|p)([^>]*>)/,
            `$1$2 style="color: ${newColorText};"$3`
          ); // Ajouter le style à la balise existante
          ...
          console.log(
            "Dans <h2>, <h3> ou <p> comportant déjà une couleur, on change la couleur."
          );
          newEndIndex = offset + p1.length;
          return p1.replace(
            /(color:\s*#[0-9A-Fa-f]{6};?)/,
          ...
          newEndIndex =
            newStartIndex + `<span style="color: #******;">${p1}</span>`.length; // Position de fin pour la sélection
  ...
  const handleBackgroundColorClick = (newColorBackgroundText) => {
    ...
    const styled =
      ...
      selectedText.includes("; color:");
    const isColorAndStyle =
      selectedText.includes("background-color: #") && styled; // background-color + au moins un autre style
    const isH2H3P =
      (selectedText.includes("<h2 ") ||
        selectedText.includes("<h3 ") ||
        selectedText.includes("<p ")) &&
      !selectedText.includes("<span ");
    const isBackColored =
      selectedText.includes("background-color: #") && !styled && isH2H3P; // Seulement background-color dans h2 ou h3 ou <p>

    if (newColorBackgroundText === "") {
      // Pour réinitialiser
      if (selectedText === "") {
        return;
      }
      ...
      if (isColorAndStyle) {
        ...
        console.log(
          "isColorAndStyle Background - couleur de fond ; la sélection inclus background-color et au moins un autre style. On retire la couleur de fond."
        );
        newText = textarea.value.replace(
          selectedText,
          selectedText.replace(/background-color:\s*#[0-9A-Fa-f]{6};?\s*/, "")
        );
      } else if (isBackColored) {
        // Si seulement background-color existe. On le retire.
        console.log(
          "(isBackColored) Dans un h2, h3 ou <p>, si seulement background-color existe et aucun autre style. On le retire"
        );
        newText = textarea.value.replace(
          selectedText,
          selectedText
      ...
      const newEndIndex =
        newStartIndex + newText.substring(startIndex, endIndex).length; // Position de fin pour la sélection
      ...
      console.log(
        `Changement de couleur de fond en : ${newColorBackgroundText}`
      );
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      ...
      if (offset === startIndex) {
        const isBackgroundColorChanged =
          selectedText.includes('<span style="background-color:') &&
          selectedText.includes("</span>");
        const isStyle =
          !p1.includes("background-color: #") &&
          (p1.includes("font-weight: bold;") ||
        ...
        const isHeadingOrParagraph =
          (p1.includes("<h2>") || p1.includes("<h3>") || p1.includes("<p>")) &&
          !p1.includes("background-color: #"); // Sans couleur
        const isH2H3Pspancolored =
          (p1.includes("<h2 ") ||
            p1.includes("<h3 ") ||
            !p1.includes("<p ") ||
            !p1.includes("<span ")) &&
        ...
        if (isBackgroundColorChanged) {
          // Si une balise <span> existe déjà, remplacez la couleur de fond
          console.log(
            "Si span contenant background-color: existe, on change la couleur"
          );
          const updatedText = p1.replace(
            /(<span[^>]*style="background-color:).*?;/,
            `$1 ${newColorBackgroundText};`
          );
          newEndIndex = offset + p1.length;
          ...
          newEndIndex =
            offset +
            p1.length +
            (`<${tagName} style="background-color: ${newColorBackgroundText};">`
              .length +
              `</${tagName}>`.length); // Ajuster la longueur pour la balise
          return p1.replace(
            /(<\/?)(h[2-3]|p)([^>]*>)/,
            `$1$2 style="background-color: ${newColorBackgroundText};"$3`
          ); // Ajouter le style à la balise existante
        } else if (isStyle) {
          // Si un (ou plusieurs) style est déjà présent, ajouter background-color: #******
          console.log(
            `isStyle ; Si un style est déjà présent, ajouter background-color: #******`
          );
          newEndIndex = offset + p1.length + 27;
          return p1.replace(
            /style="/,
            `style="background-color: ${newColorBackgroundText}; `
          );
          ...
          console.log(
            "Dans <h2>, <h3>, <p> ou <span> comportant déjà une couleur de fond, on change cette couleur."
          );
          newEndIndex = offset + p1.length;
          return p1.replace(
            /(background-color:\s*#[0-9A-Fa-f]{6};?)/,
            `background-color: ${newColorBackgroundText};`
          );
        } else {
          // Sinon, créez une nouvelle balise <span> avec la couleur de fond
          ...
          newEndIndex =
            newStartIndex +
            `<span style="background-color: #******;">${p1}</span>`.length; // Position de fin pour la sélection
    ...
    newText = textarea.value.replace(regex, (match, p1, offset) => {
      ...
      if (offset === startIndex) {
        ...
        if (alignmentRegex.test(p1)) {
          ...
          newEndIndex =
            offset +
            `<div style="text-align: ${align};">${p1.replace(
              alignmentRegex,
              "$2"
            )}</div>`.length; // Position de fin pour la sélection // Position de fin pour la sélection
          return p1.replace(
            alignmentRegex,
            `<div style="text-align: ${align};">$2</div>`
          );
        } else {
  ...
  return (
    <div className="mt-4 p-2">
      ...
      >
        {language === "fr"
          ? "Dans cette présentation, vous choisissez la mise en forme du texte"
          : "In this presentation, you choose the text formatting."}
      </p>

      <section className="py-2 ">
        <div className="boutonGris bouton" onClick={handleUndo}>
          <img className="w-[80%]" src="/icons/undo-icon.png" alt="icon undo" />
        </div>
        <div className="boutonGris bouton" onClick={handleRedo}>
          <img className="w-[80%]" src="/icons/redo-icon.png" alt="icon redo" />
        </div>
        <div
          className="longBoutonGris bouton"
        ...
        <div
          ...
          title="<h3>"
        >
          {language === "fr" ? "Sous-titre" : "Sub-title"}
        </div>
        <div
          className="longBoutonGris bouton"
          ...
          title="<p>"
        >
          {language === "fr" ? "Paragraphe" : "Paragraph"}
        </div>
        <div
          className="longBoutonGris bouton"
          ...
          title="<br/>"
        >
          {language === "fr" ? "Retour ligne" : "Line break"}
        </div>
        {/* Choisir la taille du texte ------------------------------------------------------- */}
        <select
          value={fontSize}
          className="p-1 border border-solid rounded"
        ...
        >
          ...
          <option value="1rem">
            {language === "fr" ? "Par défaut" : "By default"}
          </option>
        ...
        <div
          className="boutonGris bouton"
          onClick={handleBoldClick}
          title="<b>"
        >
          <img
            className="w-[60%]"
            src="/icons/format-bold.png"
            alt="icon bold"
          />
        </div>
        <div
        ...
        <div className="boutonTextColor bouton" onClick={toggleColorPicker}>
          ...
          <div
            className="rectangleTextColor"
            style={{ backgroundColor: rectangleUnderA }}
          ></div>
        ...
        <div
          className="boutonBackgroundColor bouton"
          onClick={toggleBackgroundColorPicker}
        >
          <div className="textColorLetter iconPencil">
            <img src="/icons/pencil-icon.png" alt="Icon pencil" />
          </div>
          <div
            className="rectangleBackgroundTextColor"
            style={{ backgroundColor: backgroundUnderPencil }}
          ></div>
        ...
        <div className="boutonGris bouton" onClick={handleAlignLeft}>
          <img
            className="w-[70%]"
            src="/icons/left-align-icon.png"
            alt="left-align-icon"
          />
        </div>
        <div className="boutonGris bouton" onClick={handleAlignCenter}>
          <img
            className="w-[70%]"
            src="/icons/align-center-icon.png"
            alt="align-center-icon"
          />
        </div>
        <div className="boutonGris bouton" onClick={handleAlignRight}>
          <img
            className="w-[70%]"
            src="/icons/right-align-icon.png"
